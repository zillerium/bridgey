...
class FabCar extends Contract {
	...
	for (let i = 0; i < gps.length; i++) {
            gps[i].docType = 'gps';
            await ctx.stub.putState('GPS' + i, Buffer.from(JSON.stringify(gps[i])));
            console.info('Added <--> ', gps[i]);
        }
    ...
    async queryGPS(ctx, gpsNumber) {
        ...
        console.log(gpsAsBytes.toString());
        return gpsAsBytes.toString();
    }
    ...
    async createGPS(ctx, gpsNumber, longitiude, latitude, userid, gpsdate {
        ...
        await ctx.stub.putState(gpsNumber, Buffer.from(JSON.stringify(gps)));
        console.info('============= END : Create gps ===========');
    }
    ...
    async queryAllGPS(ctx) {
        const startKey = '000000000';
        const endKey = 'FFFFFFFFF';
        const allResults = [];
        for await (const {key, value} of ctx.stub.getStateByRange(startKey, endKey)) {
            const strValue = Buffer.from(value).toString('utf8');
            let record;
